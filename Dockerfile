...
RUN yum install -y git perl java tar mysql-connector-java
ADD http://www.atlassian.com/software/stash/downloads/binary/atlassian-stash-$STASH_VERSION.tar.gz /opt/atlassian-stash.tar.gz
RUN mkdir $STASH_INST
RUN tar xzvf /opt/atlassian-stash.tar.gz -C $STASH_INST --strip-components=1
RUN ln -s /usr/share/java/mysql-connector-java.jar $STASH_INST/lib
